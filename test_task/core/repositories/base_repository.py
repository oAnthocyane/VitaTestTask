...
from test_task.core.schemas import (
    CreateBaseModel,
...
class BaseRepositoryProtocol(
    ...
    async def get_by_ids(
        self: Self, ids: Sequence[uuid.UUID]
    ) -> list[ReadSchemaType]: ...
...
class BaseRepositoryImpl(
        ...
        try:
            ...
            if (
                ...
                raise ModelAlreadyExistsError(
                    self.model_type,
                    field_name,
                    f"duplicate key for field: {field_name}",
                )
            raise
    ...
    async def bulk_create(
        ...
        async with self.session as s, s.begin():
            ...
            models = (
                await s.scalars(statement, [x.model_dump() for x in create_objects])
            ).all()
            return [
                self.read_schema_type.model_validate(model, from_attributes=True)
                for model in models
    ...
    async def bulk_update(self, update_objects: list[UpdateSchemaType]) -> None:
        ...
        async with self.session as s, s.begin():
            ...
            await s.execute(
                statement, [x.model_dump(exclude_unset=True) for x in update_objects]
            )
        ...
        async with self.session as s, s.begin():
            ...
            statement = (
                ...
                .on_conflict_do_update(
                    ...
                    set_=create_object.model_dump(exclude={*primary_keys}),
                )
                .returning(self.model_type)
            )
    ...
    def get_order_by_expr(
        self: Self, sorting: Iterable[str]
    ...
    def _extract_field_name(
        self, error_message: str, create_schema_type: type[CreateSchemaType]
    ) -> str:
